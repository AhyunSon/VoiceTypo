...
**`python main.py` 로 실행되며, 실시간 모음 인식이 실제로 동작하는 완성된 앱입니다.**
...
실시간 모음 인식·시각화·캘리브레이션은 **정상 동작**하는 완성된 앱.
다만 오프라인 정확도 평가에서는 **약 22.9%** (02_formant 진단 기록 기준) — 정확도를 더 끌어올릴 여지가 컸음.
...
동작하는 초기 버전 — 실시간 인식·GUI·캘리브레이션 작동.
02_formant 의 출발점이며, 정확도 개선 실험은 거기서 이어짐 (Phase A/B, 최신 92.9%).
