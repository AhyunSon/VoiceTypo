...
def sdf_to_rgba(sdf, color_top, color_bot, opacity=1.0):
    ...
    if opacity < 1.0:
        work *= opacity
    alpha = work.copy()  # alpha 보존 (채널 계산에서 work 재사용)

    # R, G, B 채널: color = top*(1-t) + bot*t, premultiply by alpha
    for i in range(3):
        np.multiply(color_top[i], 1.0 - t, out=work)
        work += color_bot[i] * t
        work *= alpha
