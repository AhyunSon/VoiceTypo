...
| 용도 | 데이터 | 내용 |
| --- | --- | --- |
| 학습 | Zeroth-Korean | 모음 28000 샘플 / 104 화자 / 7모음 각 4000개 균형 |
| 평가 (미관찰 화자) | Pansori-TEDxKR | 41 화자 / 14700 세그먼트 — 학습에 한 번도 안 씀 |
| 확장 후보 (아직 미사용) | Common Voice ko, FLEURS-ko | 화자 다양성 확대용 — 다음 단계 투입 예정 |
| 증강 | MUSAN noise, RIR | 잡음·잔향 추가 |
| 모음 구간 정렬 | Wav2Vec2-XLSR-Korean | CTC 강제정렬로 모음 구간만 잘라냄 |
| 인코더 (사전학습) | Whisper-base (290MB) | frozen — 특징 추출용, 학습 안 함 |

**화자 분리 split (Zeroth):** train 21719(78화자) / val 1927(14화자) / test 4354(12화자).
세 셋이 화자를 공유하지 않음 — 일반화 측정의 핵심.

**모음 샘플 만드는 흐름:** 코퍼스 음성 → Wav2Vec2-XLSR CTC 강제정렬 →
자모 분해로 모음 위치 탐색 → 모음 안정 구간(30~90%)만 슬라이스 → 28000 샘플.
